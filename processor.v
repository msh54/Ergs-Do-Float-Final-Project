...
module processor(
    ...
    /* YOUR CODE STARTS HERE */
	 ...
	 //---------------------------------PC---------------------------------
		...
		wire jumpNoBranch;
		and _checkForBranch(jumpNoBranch,takeJump,(takeBranch == 0));
		mux_2to1_12 _takeJump(jumpNoBranch,pc_input,pc_jump,pc_norm);
		
		wire stall_multdiv;
		wire stall_dx; // referenced in Decode control
		
		pc_register _pc(address_imem, pc_input,stall_multdiv,clock,reset);
		//GOES INTO IMEM
		// Imem: output [11:0] address_imem; input [31:0] q_imem;
		add1 _increment(pc_postAdd,address_imem);
		
		wire flush_fd_reg,stall_fd_reg;
		or _flush_fd_reg(flush_fd_reg,takeBranch,takeJump);
		or _stallFd(stall_fd_reg,stall_dx,stall_multdiv);
		//fd_reg _fd_reg(out: fd_out, pc_output_fd, input: q_imem,pc_postAdd,.flush(takeBranch),control: .disable(stall)
		fd_reg _fd_reg(fd_out,pc_output_fd,q_imem,address_imem,flush_fd_reg,stall_fd_reg,clock,reset);
	 ...
	 //---------------------------------Decode Stage---------------------------------
		...
		//module control(notNop,loadStatus,MemRead,writeReg_in_decode,ctrl_dataIn,branch_bne,branch_blt,branch_bex,MemtoReg,ALUOp,MemWrite,ALUSrc,RegWrite,stall_dx,opCode,dxRd,fdRt,fdRs,ALUOp_In,loadRd,dxMemRead);
		    control ctrl(multdiv,notNop,loadStatus,MemRead,writeReg_in_decode,ctrl_dataIn,branch_bne,branch_blt,branch_bex,MemtoReg,ALUOp,MemWrite,ALUSrc,RegWrite,stall_dx,fd_out[31:27], regDst_from_dx, fd_out[16:12],fd_out[21:17],fd_out[6:2],loadRd,MemRead_from_dx);
		...
		wire multdiv_from_dx,stall_dx_reg;
		or _stallDx(stall_dx_reg,stall_dx,stall_multdiv);
		wire ALUSrc_from_dx, MemWrite_from_dx, bne_from_dx,blt_from_dx,bex_from_dx,MemToReg_from_dx, RegWrite_from_dx;
		//module dx_reg(multdiv_from_dx,pc_output_dx, dataA_from_dx, dataB_from_dx, 
			...
			// xtendOut, regDst,ctrl_readRegA_from_dx,ctrl_readRegB_from_dx,
			// data_readRegA,data_readRegB,multdiv,pc_output_fd,
			// ALUSrc, ALUOp, MemWrite, branch_bne, 
			// branch_blt, branch_bex,MemToReg,RegWrite,flush,stall_dx,ctrl_readRegA,ctrl_readRegB,MemRead,clock,reset);
		dx_reg _dx_reg(multdiv_from_dx,pc_output_dx, dataA_from_dx, dataB_from_dx, 
				imm_from_dx, regDst_from_dx, ALUSrc_from_dx, 
				ALUOp_from_dx, MemWrite_from_dx,bne_from_dx,
				blt_from_dx,bex_from_dx, MemToReg_from_dx, RegWrite_from_dx,MemRead_from_dx,
				...
				data_readRegA,data_readRegB,multdiv,pc_output_fd,
				ALUSrc, ALUOp, MemWrite, branch_bne, 
				branch_blt,branch_bex, MemtoReg, RegWrite,takeBranch,stall_dx_reg,ctrl_readRegA,ctrl_readRegB,MemRead,clock,reset);
	 ...
	 //---------------------------------Execute Stage---------------------------------
	   //control(regDst,Branch,MemRead,MemtoReg,ALUOp,MemWrite,ALUSrc,RegWrite,inst);
		...
		wire bypass_dmem,notABranch,RegWrite_from_xm;
		
		nor _notABranch(notABranch,bne_from_dx,blt_from_dx);
		//assign bypass_A = 2'b0; // 00 = Nothing, 01 = MX = (ctrl_readRegA_from_dx == regDst_from_xm), 10 = WX = (ctrl_readRegA_from_dx == regDst_from_mw)
		and _andIt(bypass_A[1],(ctrl_readRegA_from_dx == regDst_from_mw && RegWrite_from_mw == 1'b1),(ctrl_readRegA_from_dx != regDst_from_xm || RegWrite_from_xm == 1'b0),(regDst_from_xm != 5'd0));
		and(bypass_A[0],(ctrl_readRegA_from_dx == regDst_from_xm && RegWrite_from_xm == 1'b1),(regDst_from_xm != 5'd0));
		// assign bypass_B = 2'b0; // 00 = Nothing, 01 = MX = (ctrl_readRegB_from_dx == regDst_from_xm), 10 = WX = (ctrl_readRegB_from_dx == regDst_from_mw)
		//wire testOne;
		//assign testOne = 1'b1;
		
		and(bypass_B[1],(ctrl_readRegB_from_dx == regDst_from_mw && RegWrite_from_mw == 1'b1),(ctrl_readRegB_from_dx != regDst_from_xm || RegWrite_from_xm == 1'b0),(regDst_from_mw != 5'd0));
		//and _and1(bypass_B[1],testOne,regDst_from_mw);
		and(bypass_B[0],(ctrl_readRegB_from_dx == regDst_from_xm && RegWrite_from_xm == 1'b1),(regDst_from_xm != 5'd0));
		...
		wire[31:0] aluOutput;
		wire isNotEqual, isLessThan, isLessThan_ALU, overflow;
		
		
		
		wire w1,w2,w3,w4,w5,w6,w7,w8,w9,w10;
		dffe_ref _cycle1(w1,multdiv_from_dx,clock, 1'b1, 1'b0);
		dffe_ref _cycle2(w2, w1, clock, 1'b1, 1'b0);
		dffe_ref _cycle3(w3, w2, clock, 1'b1, 1'b0);
		dffe_ref _cycle4(w4, w3, clock, 1'b1, 1'b0);
		dffe_ref _cycle5(w5, w4, clock, 1'b1, 1'b0);
		dffe_ref _cycle6(w6, w5, clock, 1'b1, 1'b0);
		dffe_ref _cyclet(w7, w6, clock, 1'b1, 1'b0);
		dffe_ref _cycle7(w8, w7, clock, 1'b1, 1'b0);
		dffe_ref _cycle8(w9, w8, clock, 1'b1, 1'b0);
		dffe_ref _cycle9(w10, w9, clock,1'b1, 1'b0);	
		
		xor _stalling(stall_multdiv,multdiv_from_dx,w1,w2,w3,w4,w5,w6,w7,w8,w9,w10);
		
		
		alu _mainAlu(aluDataAsource, aluDataBsource,ALUOp_from_dx,imm_from_dx[11:7],aluOutput,isNotEqual,isLessThan_ALU,overflow);
		
		assign isLessThan = isLessThan_ALU;
		//not _isLessThan(isLessThan,isLessThan_ALU);
